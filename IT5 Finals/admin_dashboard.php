...
$election_status_counts = [
    $db->query("SELECT COUNT(*) FROM elections WHERE status = 'draft'")->fetchColumn(),
    $db->query("SELECT COUNT(*) FROM elections WHERE status = 'active'")->fetchColumn(),
    $db->query("SELECT COUNT(*) FROM elections WHERE status = 'closed'")->fetchColumn(),
    $db->query("SELECT COUNT(*) FROM elections WHERE status = 'archived'")->fetchColumn(),
];
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-gray-50">
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 text-white flex flex-col overflow-y-auto" style="background: linear-gradient(180deg, #1565C0 0%, #1e90ff 100%);">
            <div class="p-6 border-b border-blue-400 border-opacity-40">
                <div class="flex items-center gap-3">
                    ...
                    <div><h1 class="font-bold text-lg">ADMIN</h1></div>
                </div>
            </div>
            <nav class="flex-1 py-6 space-y-1">
                ...
                <a href="ballot_builder.php" class="sidebar-link flex items-center gap-3 px-6 py-3 text-sm font-medium text-slate-300 hover:text-white">
                    <i data-lucide="file-text" class="w-5 h-5"></i> Ballot Builder
                </a>
                <a href="voters.php" class="sidebar-link flex items-center gap-3 px-6 py-3 text-sm font-medium text-slate-300 hover:text-white">
                    <i data-lucide="users" class="w-5 h-5"></i> Voter Management
                </a>
                ...
                <a href="users.php" class="sidebar-link flex items-center gap-3 px-6 py-3 text-sm font-medium text-slate-300 hover:text-white">
                    <i data-lucide="user-cog" class="w-5 h-5"></i> User Management
                </a>
        ...
        <main class="flex-1 overflow-y-auto" id="mainContent">
            ...
            <div class="p-8 space-y-6">
                    ...
                    <div class="stat-card bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1"><?php echo $turnout; ?>% Turnout</p>
                            </div>
                ...
                <!-- Recent Elections & System Status -->
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <a href="elections.php" class="flex items-center gap-2 p-2 text-sm text-gray-700 hover:bg-white rounded border border-gray-200 hover:shadow-sm transition">
                                        <i data-lucide="plus-circle" class="w-4 h-4 text-blue-600"></i> New Election
                                    </a>
                                    <a href="voters.php" class="flex items-center gap-2 p-2 text-sm text-gray-700 hover:bg-white rounded border border-gray-200 hover:shadow-sm transition">
                                        <i data-lucide="upload" class="w-4 h-4 text-green-600"></i> Import Voters
                                    </a>
                                    <a href="admin_results.php" class="flex items-center gap-2 p-2 text-sm text-gray-700 hover:bg-white rounded border border-gray-200 hover:shadow-sm transition">
                                        <i data-lucide="download" class="w-4 h-4 text-blue-600"></i> Export Results
                                    </a>
                                    <a href="notifications.php" class="flex items-center gap-2 p-2 text-sm text-gray-700 hover:bg-white rounded border border-gray-200 hover:shadow-sm transition">
                                        <i data-lucide="send" class="w-4 h-4 text-purple-600"></i> Send Alert
                                    </a>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        new Chart(document.getElementById('electionChart').getContext('2d'), {
            type: 'doughnut',
            data: {
                labels: ['Draft', 'Active', 'Closed', 'Archived'],
                datasets: [{
                    data: <?php echo json_encode($election_status_counts); ?>,
        ...
        // Reset scroll after charts render
