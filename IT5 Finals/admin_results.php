...
if ($election_id) {
    ...
    if ($election) {
        $stmt = $db->prepare("SELECT p.* FROM positions p WHERE p.election_id = ? ORDER BY p.id");
        $stmt->execute([$election_id]);
    ...
    if ($format === 'csv' && $election) {
        ...
        foreach ($positions as $pos) {
            foreach ($pos['candidates'] as $cand) {
                ...
                fputcsv($output, [$pos['title'], $cand['name'], $cand['party'] ?? 'N/A', $cand['vote_count'], $pct . '%']);
            }
        }
        fclose($output);
        exit();
...
<body class="bg-gray-50">
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 text-white flex flex-col overflow-y-auto" style="background: linear-gradient(180deg, #1565C0 0%, #1e90ff 100%);">
            ...
            <nav class="flex-1 py-6 space-y-1">
                ...
                <a href="admin_results.php" class="sidebar-link active flex items-center gap-3 px-6 py-3 text-sm font-medium text-white">
            ...
            <div class="p-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    ...
                    <form method="GET" class="flex gap-3">
                        <select name="election_id" id="electionSelect" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
    ...
    <div style="position: relative; height: 250px;">
        <canvas id="chart_<?php echo $position['id']; ?>"></canvas>
    </div>
</div>
                                <div>
                                    ...
                                    <div class="space-y-3">
                                        <?php 
                                        $max_votes = !empty($position['candidates']) ? max(array_column($position['candidates'], 'vote_count')) : 0;
                ...
                <div class="text-center py-12 bg-white rounded-xl border border-gray-200">
                    ...
                    <a href="ballot_builder.php?election_id=<?php echo $election_id; ?>" class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                        <i data-lucide="file-plus" class="w-4 h-4"></i> Build Ballot
                    </a>
                </div>
                <?php else: ?>
                <div class="text-center py-12 bg-white rounded-xl border border-gray-200">
    ...
    <!-- All scripts at the very bottom -->
    <script>
        // Initialize icons first
        lucide.createIcons();

        // Then render charts without causing scroll
        <?php if ($election && !empty($positions)): ?>
        (function() {
            var chartData = <?php
                $chartDataArr = [];
                foreach ($positions as $position) {
                    $chartDataArr[] = [
                        'id' => $position['id'],
                        'labels' => array_map(function($c) { return $c['name']; }, $position['candidates']),
                        'data' => array_map(function($c) { return (int)$c['vote_count']; }, $position['candidates']),
                    ];
                }
                echo json_encode($chartDataArr);
            ?>;

            chartData.forEach(function(pos) {
                var canvas = document.getElementById('chart_' + pos.id);
                if (!canvas) return;
            ...
            // Lock scroll position to top after charts render
